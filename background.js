...
//       // Store token for use by content scripts / popup
//       chrome.storage.local.set({ authToken: authHeader.value });
//       // For debugging:
//       // console.log("Captured Authorization:", authHeader.value);
//     }
//   },
//   {
//     // IMPORTANT: include every API host that sends the Authorization header
//     urls: ["https://api.examly.io/*"]
//   },
//   // We need requestHeaders to read them, and extraHeaders to access sensitive ones like Authorization
...
chrome.webRequest.onBeforeSendHeaders.addListener(
  (details) => {
    ...
    if (authHeader && authHeader.value !== authToken) {
      authToken = authHeader.value;
      chrome.storage.local.set({ authToken });
    //   console.log("✅ Token captured:", authToken);

      // Fetch QB names once token is available
      fetchQbNames(authToken);
    }
...
// async function fetchQbNames(token) {
//   try {
//     const res = await fetch("https://api.examly.io/api/v2/tests/filter", {
//       method: "POST",
//       headers: {
...
//     // Extract qb_name for Rule Based Test
//     const qbNames = [];
//     (data?.data || []).forEach(test => {
//       if (test.testType === "Rule Based Test") {
//         test.test_rules?.forEach(ruleGroup => {
//           ruleGroup.forEach(blockArr => {
//             blockArr.forEach(block => {
//               block.questionbank?.forEach(qb => {
//                 if (qb.qb_name) qbNames.push(qb.qb_name);
//               });
//             });
//           });
//         });
...
//     console.log("📘 QB Names:", qbNames);
//     chrome.storage.local.set({ qbNames });
//   } catch (err) {
//     console.error("❌ Fetch QB names failed:", err);
//   }
// }
...
async function fetchQbNames(token) {
  try {
    let page = 1;
    const limit = 50;   // safer, backend may cap at 25/50
    let allQbNames = [];
    let hasMore = true;
    ...
    while (hasMore) {
      ...
      // Debug: log how many tests we got
      console.log(`Page ${page}: got ${data?.data?.length || 0} tests`);
        ...
        if (test.testType === "Rule Based Test") {
          test.test_rules?.forEach(ruleGroup => {
            ruleGroup?.forEach(blockArr => {
              blockArr?.forEach(block => {
                block.questionbank?.forEach(qb => {
                  if (qb.qb_name) allQbNames.push(qb.qb_name);
                });
              });
            });
          });
      ...
      // If fewer than limit, last page reached
    ...
    console.log("📘 Total QB Names fetched:", allQbNames.length);
    chrome.storage.local.set({ qbNames: allQbNames });
